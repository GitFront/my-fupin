...
<head>
	...
	<script type="text/javascript">
		...
		function initForm(){
			/* 显示表单 */
			$("#mappingName").textbox('setValue','${query.tableMappingName}');
			/* 配置保存表单 */
	...
	<script type="text/javascript">
		...
		/* 打开个人路径编辑窗口 */
		function openEditCatalogTree(catalogId){
		...
		$(document).ready(function(){
			...
			function usePath(){
				...
				$('#'+currentCatalogId).combotree('setValue',selected);
	...
	<script  type="text/javascript">
		...
		var noselectuser = {
		    	...
		    	columns:[
		    	   {field : 'id',hidden:true}, 
		    	   {width : '60',title : '用户名',field : 'username'}, 
		    	   {title : '所在机构',field : 'dept'}]
		};
		...
		$(function(){
			new TwoWindowsSelecter("share_usr",{title:"数据文件分享",width:700,height:300},noselectuser,hasselectuser);
			$("#sharer").hide();
			$('#saveDataWin').dialog('close');
 ...
 <body>
								...
								<div style="margin-top: 10px">   
							 	...
							 	<div style="margin-top: 10px">   
							  	...
							  	<div style="margin-top: 10px">   
							        ...
							        <span><a id="catalog_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-btn-plus-sign'" onclick="javascript:openEditCatalogTree('personalCatalogId');">新增目录</a></span> 
							  	</div>
							  	<div style="margin-top: 10px">
		...
		<div id="editPathWin" class="easyui-dialog" title="编辑个人目录"    
		        ...
		        buttons:[{text:'选 取',iconCls:'icon-btn-ok',handler:usePath},{text:'取 消',iconCls:'icon-btn-remove',handler:function(){$('#editPathWin').dialog('close');}}]">   
		        <div >
		...
		<div id="saveDataWin" class="easyui-dialog" title="保存数据文件" 
		        data-options="iconCls:'icon-btn-hdd',resizable:false,modal:true,draggable:true,
		        buttons:[{text:'保 存',iconCls:'icon-btn-hdd',handler:saveData},{text:'取 消',iconCls:'icon-btn-remove',handler:function(){$('#saveDataWin').dialog('close');}}]">   
		        <div style="padding:15px;" >
					<form id="saveFileData" method="post">
						<div style="margin-top: 10px">   
							 <label for="savefileName">文件名称：</label>   
							 <input class="easyui-textbox" type="text" name="savefileName" style="width:160px;" data-options="required:true"/> + 当前时间
						</div>
						<div style="margin-top: 10px">   
							 <label for="personalCatalogId2">文件路径：</label>
							 <span><select id="personalCatalogId2" name="personalCatalogId2" class="easyui-combotree" data-options="required:true" style="width:160px;"></select></span>
							 <span><a id="personal_catalog_btn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-btn-plus-sign'" onclick="javascript:openEditCatalogTree('personalCatalogId2');">新增目录</a></span> 
						</div>
					    <div style="margin-top: 10px">
					        <a id="isShare" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-btn-share',toggle:true,plain:false" onclick="javascript:shareSwitch();">启动数据分享</a>  
					    </div>
					    <fieldset id="sharer" style="width:700px;height:310px;margin-top: 10px;">
							<legend>分享数据配置</legend>
