   ...
   /*对外方法 */
    $.poorDetail={
        ...
        showCountry:function(country_id){
            ...
            renderPoorCountry(country_id);
        },
        showFamily:function(family_id, tabClass){
            $('.pop-win-wrap').show();
            $('.file-detail').hide();
            var $poorFamilyDetail = $('#poorFamilyDetail').show();
            if(tabClass){
                $poorFamilyDetail.find('[data-tab=' + tabClass + ']')
                  .addClass('active')
                  .siblings()
                  .removeClass('active');
                $('.'+tabClass).show()
        ...
        TABS_FAMILY:{
            FILE:'js-family-file',
            HELP_INFO:'js-family-help-info',
            SUCCESS_INFO:'js-family-success-info',
            ALBUM:'js-family-album-box'
        }
    };
     ...
     * @param country_id
     */
    function renderPoorCountry(country_id){
        var _url=ctx+'/data/country/'+country_id+'.json';
        $.ajax({
            url:_url,
            dataType:'json',
    ...
    function renderPoorFamily(family_id){
        var _url=ctx+'/data/family/'+family_id+'.json';
        $.ajax({
            url:_url,
            dataType:'json',
            success:function(data){
