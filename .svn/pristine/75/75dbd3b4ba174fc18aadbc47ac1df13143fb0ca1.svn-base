   ...
   var queryName="com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryEcMapData";
    $(function(){
    	...
    	if(ret){ret.then(function(res){
    		  ...
    		  helppac = res.helppac;
    		  Highmap.mybatis.Map("map",queryName,{"type":type,"scope":scope,"month":statTime ,"helppac":helppac},
    		  	{key:'PAC',value:'COMPLETE_MODEL_NUM',value2:'NATURAL_NUM',value3:'COMPLETE_MODEL_RATE'},"PAC:"+res.mapPac,"PAC",option_);
    ...
    function loadMap(type){
		...
		Highmap.mybatis.reloadData(Highmap.mapid,queryName,{"type":type,"scope":scope,"month":statTime,"helppac":helppac},mapper,valueMapper,colorStops);
    }
     ...
     function re(e){
	     ...
	     if(pointType=='PKC'){
	       	$.poorFile.showCountryPhoto(pac,"pop");
	      }
     }
     function test(){
    	 alert(1)
     ...
     function showpoint(pac,level){
    	var loader = {};
		 if(level < 5) {
			 ...
			 if(level > 1) radius = 3;
			 var point1 = {
					 queryName:'com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryMappiontGeoForCountry', /*通过远程方式读取，读取的查询名称（mybatis）读取的查询参数为pac:区域ID，level:区域等级 */
					 parameters:{"month":statTime,"buildStatus":"1","helppac":helppac},
			 ...
			 var point2 = {
					 queryName:'com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryMappiontGeoForCountry', /*通过远程方式读取，读取的查询名称（mybatis）读取的查询参数为pac:区域ID，level:区域等级 */
					 parameters:{"month":statTime,"buildStatus":"0","helppac":helppac},
					 mapper:{name:'NAME',PAC:'PAC',PPAC:'PPAC',longitude:'LONGITUDE',latitude:'LATITUDE',type:'POINT_TYPE'}, /*通过远程方式读取，读取的查询转义*/
