...
public interface ReportService {
	...
	/**
	 * 根据pac查询下级或同级的地图geojson数据
	 * @param queryName mybatis的查询名称（需要查询数据为：区域中文名、区域ID、父级区域ID、下钻区域ID、区域经纬度点信息）
	 * @param params 查询参数
	 * @param mapper 参数名称对应集合(mapKey,name,parentKey,drillKey,geometry)
	 * @throws Exception 
	 */
	public	MapGeometryVo  queryMapGeometry(String queryName,Map<String, Object> params,Map<String,Object> mapper);
	
	/**
	 * 根据pac查询下级或同级的地图geojson数据
	 * @param queryName mybatis的查询名称（需要查询数据为：位置点中文名、位置点ID、父级区域ID、位置点经度、位置点纬度信息）
	 * @param params 查询参数
	 * @param mapper 参数名称对应集合(mapKey,name,parentKey,longitude,latitude)
	 * @throws Exception 
	 */
	public	List<MapGeometryVo>  queryMappointGeometry(List<String> queryName,List<Map<String, Object>> params,List<Map<String,Object>> mapper);
