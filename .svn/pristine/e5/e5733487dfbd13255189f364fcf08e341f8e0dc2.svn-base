...
var Highmap = {
	...
	loadPointGeoUrl:'/a/report/highmaps/pointGeo/load',
	  ...
	  MapOptionTemplates: {
	  	   ...
	  	   * @return {}
	  	   */
	      Map: function (container,data,mapData,joinBy,opt,callback) {
	          ...
	          Highmap.pointFormat = Highmap.utils.convValueFormat(opt);
	          Highmap.utils.createMapSeries(opt.mapName,mapValue_[Highmap.topMapNode],mapData,joinBy,opt,doOption);
	          
	          function doOption(series){
	          	var option = {
			        chart : {
			        	height:opt.height,
			        	/*width:opt.width,*/
			                ...
			                drilldown: function (e) {
			                	if (!e.seriesOptions) {
					                ...
					                if(Highmap.isLayerLoad && Highmap.valueSource === "Ajax"){
					                	...
					                	parameter['map_level'] = Highmap.utils.getMapLevel(key);
					                	var map_pac_level = '';
									...
									if(common.utils.isFunction(funcName)){
										var lvl = Highmap.utils.getMapLevel(key);
										eval(funcName + "(e.point,key,lvl)");
									}
				            	}
			                },
			                drillup: function (e) {
								...
								if(common.utils.isFunction(funcName)){
									var lvl = Highmap.utils.getMapLevel(Highmap.levelid);
									eval(funcName + "(Highmap.levelid,lvl)");
								}			
			                }
			            }
			        },
				            ...
				            events: {
				            	click: function (e) {
				            		if (!e.seriesOptions) {
										...
										if(common.utils.isFunction(funcName)){
											var lvl = Highmap.utils.getMapLevel(Highmap.levelid);
											eval(funcName + "(e.point,Highmap.levelid,lvl)");
										}
				            		}
				            	}
				            }
	  ...
	  create:{
	  	 ...
	  	 * @return {}
	  	 */
	  	init:function(container,data,mapData,joinBy, option,isNotLoading,callback){
			...
			if(Highmap.valueSource === "Static")
	  		...
	  		eval("Highmap.MapOptionTemplates."+type+"(container,data,mapData,joinBy,option_,initMap)");
	  		...
	  		function initMap(opt){
			    ...
			    var pointInfo = Highmap.utils.loadMappoint(option_,Highmap.topMapNode,Highmap.utils.getMapLevel(Highmap.topMapNode));
		    	...
		    	if(common.utils.isFunction(callback)) eval("callback()");
	  		}
	  	}
	  ...
	  reload: function (chartid,data,newOption,isNotLoading) {
	      ...
	      if(obj != null){
			  ...
			  eval("Highmap.MapOptionTemplates."+type+"(container,data,mapData,joinBy,opt,reloadMap)");
			  
			  function reloadMap(option){
			  	if(!isNotLoading) Highmap.utils.mapHideLoading();
				  /*销毁图表对象 */	
				  chart.destroy();
	  ...
	  ajax:{
	  	...
	  	init:function(container,url,parameters,mapData,joinBy,option){
	  		...
	  		/*创建空的报表，并获取报表对象*/
	  		Highmap.create.init(container,[],mapData,joinBy,option,true,reloadAjax);
	  		function reloadAjax(){
	  			var obj = common.report.highcharts.object.get(Highmap.mapid);
		  		var chart = obj.chart;
				...
				if(option.isLayerLoad){
					...
					parameters['map_level'] = Highmap.utils.getMapLevel(Highmap.topMapNode); 
		  		...
		  		$.ajax({
		             ...
		             success: function(data){
		             	/*动态加载数据*/
		             	Highmap.reloadData(Highmap.mapid,data,undefined,undefined,true);
		                Highmap.utils.mapHideLoading();
		                /*管理报表对象信息*/
		                obj = common.report.highcharts.object.get(Highmap.mapid);
				        obj["url"] = url;
				        obj["parameters"] = parameters;
		                common.report.highcharts.object.reset(Highmap.mapid,obj);
		             }
		         });
	  		}
		    ...
		    if(obj != null){
		    	if(common.utils.isEmpty(url)){
					...
					if(Highmap.isLayerLoad){
							...
							parameters['map_level'] = Highmap.utils.getMapLevel(Highmap.levelid);
							var map_pac_level = '';
							for (var index = 0; index < Highmap.levarray.length; index++) {
									if(index === 0){
										map_pac_level = Highmap.levarray[index].id;
		 ...
		 * @param {} option 自定义配置项
	  	...
	  	Map:function(container,queryName,parameters,mapper,mapData,joinBy,option){
			...
			params.joinBy = joinBy;
	  		...
	  		Highmap.ajax.init(container,url,params,mapData,joinBy,option);
	  	},
	  ...
	  utils:{
		  ...
		  defaultOption:function(option){
			...
			var defaultMapLoader = {
				...
				isDomain:true,  /*控制localUrl是否为本地资源*/
				jsonpCallback:'mapGeoJson', /*访问跨域JSON文件的默认定义名（mapGeoJson）*/
			...
			var defaultPointLoader = {
		  		...
		  		isDomain:true,  /*控制localUrl是否为本地资源*/
				jsonpCallback:'pointGeoJsonp', /*访问跨域JSON文件的默认定义名*/
		 ...
		 * @return {}
		 */
		  getMapLevel:function(mapKey){
		  	var mapLevel = 5;
			if(mapKey != null && mapKey.length === 12){
				if(mapKey.substring(2,4) === "00"){
					mapLevel = 1;
				}else if(mapKey.substring(4,6) === "00"){
					mapLevel = 2;
				}else  if(mapKey.substring(6,9) === "000"){
					mapLevel = 3;
				}else if(mapKey.substring(9,12) === "000"){
					mapLevel = 4;
				}
			}
			return mapLevel;
		   ...
		   /**
		   * 获取地图GIS数据
		   * @param {} mapKey 地图数据的主键信息
		   * @param {} type 主键类型（key：预加载地图数据的主键，pac：通过地图区域编码获取mapLoader）
		   * @param {} mapLoader 地图数据加载属性
		   * @param {}  callback 回调函数（返回的生成地图JSON）
		   */
		  getGeoJson:function(mapKey,type,mapLoader, callback){
		  	var geoJson = '';
		  	if(type === 'key'){
		  		/*mapData方式读取（预加载）*/
		  		/*geoJson = Highcharts.geojson(Highcharts.maps[mapKey]);*/
		  		eval("callback(Highcharts.maps[mapKey])");
		  	}else{
		  		/* 定义ajax读取参数 */
		  		var url = '';
				var parmeters = {};
				if (mapLoader.isRemote) {
					/*获取动态数据读取*/
					if('ajax' === mapLoader.remoteType){
						url = ctx + mapLoader.remoteUrl;
						parmeters = {
							"map_pac" : mapKey,
							"map_level" : Highmap.utils.getMapLevel(mapKey)
						};
						Highmap.utils.loadAuthParameters(parmeters);
					}else{
						url = ctx + Highmap.loadMapGeoUrl;
						var param_ = { "map_pac":mapKey, "map_level":Highmap.utils.getMapLevel(mapKey) };
						Highmap.utils.loadAuthParameters(param_);
						parmeters = {
							queryName : mapLoader.commonQueryName,
							param : param_,
							mapper : mapLoader.commonMapper
						};
					}
					$.ajax({type : "GET", url : url, data : parmeters, async:false, 
						contentType : "application/json; charset=utf-8",
						dataType : 'json', crossDomain : true,
						success : callback, error : egeojson
					  }); 
				
				} else {
					/*获取静态数据读取*/
					if(mapLoader.isDomain){
						/*本域读取*/
						url = ctx + mapLoader.localUrl + mapKey + ".geo.json";
						$.ajax({
						...
						/*跨域读取*/
						url = mapLoader.localUrl + mapKey + ".geo.json";
						$.ajax({
					        type : "GET", url : url, async:false, 
					        dataType : "jsonp",
					        jsonpCallback:mapLoader.jsonpCallback,
							success : callback, error : egeojson
						});
					} 
				}
		  	...
		  	/*function sgeojson(json){
		  		geoJson = Highcharts.geojson(json);
		  		if(geoJson === '') 
		  			common.utils.showErrorMsg('无法正常加载地图数据，请联系系统管理员处理！',function(){return;});
		  		return geoJson;
		  	}*/
		  ...
		  mapDrilldown:function(e,data,opt){
	        ...
	        var level = Highmap.utils.getMapLevel(key);
			var cname = e.properties["name"];
			Highmap.utils.mapShowLoading();

			var geoJson = Highmap.utils.getGeoJson(dkey, 'pac', opt.mapLoader,doDrilldown);
			
			/**
			 * 地图下钻操作
			 */
			function doDrilldown(mapJson){
				geoJson = Highcharts.geojson(mapJson);
		   ...
		   * @return {}
		   */
		  createMapSeries:function(mapName,data,mapData,joinBy,opt,callback){
		  	 var series = {};
		  	 var mapKey = mapData;
		  	 var type = 'key';
		  	 /*更新顶级节点信息*/
		  	 if(mapData.substring(0,4) === 'PAC:'){
		  	 	mapKey = mapData.substring(4,mapData.length);
		  	 	type = 'pac';
		  	 	Highmap.topMapNode = mapKey;
		  	 }
		  	 var geoJson = Highmap.utils.getGeoJson(mapKey,type,opt.mapLoader,doCreate);
		  	 
		  	 function doCreate(mapJson){
		  	 	 geoJson = Highcharts.geojson(mapJson);
		  	 	 Highmap.topMapJson = geoJson;
			  	 var data_ = Highmap.ChartDataFormate.FormatGeoData(geoJson,data);
