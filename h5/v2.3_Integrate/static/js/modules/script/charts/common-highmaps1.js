	...
	topMapNode:'',
		                ...
		                drilldown: function (e) {
		                	if (!e.seriesOptions) {
								...
								if(common.utils.isFunction(funcName)){
									eval(funcName + "(key,Highmap.levarray.length)");
								}
			            	}
		                },
		                drillup: function (e) {
		                    ...
		                    if(Highmap.level.point.enable){
		                    	for (var index = 0; index < Highmap.levarray.length; index++) {
		                    		if(Highmap.mapObject.series[index].userOptions.id === Highmap.level.point.id){
		                    			Highmap.mapObject.series[index].remove(false);
		                    			break;
		                    		}
		                    	}	         	
		                    }		                    	
	  ...
	  create:{
	  	...
	  	init:function(container,data,mapData,joinBy, option){
	    	...
	    	/*动态生成一个首层默认的标识ID*/
	    	var pointid = common.utils.guid();
	    	/*保存当前的层级ID及地图属性全路径*/
	    	Highmap.levelid = Highmap.topMapNode;	
	    	Highmap.level = {
		    	id:Highmap.topMapNode,
		    	name:option_.mapName,
		    	point:{enable:option_.pointLoader.isLoadPoint,id:pointid},
		    	geojson:Highcharts.maps[mapData].features
	    	};
	    	Highmap.levarray.push(Highmap.level);
	    	
	    	if(option_.pointLoader.isLoadPoint)
		    	Highmap.utils.loadTopMappoint(option_,pointid);
	   ...
	   * @return 是否成功更新对象
	   */
	  reloadData: function (chartid,data,valueMapper) {
	  ...
	  ajax:{
	  	 ...
	  	 * @return {}
	  	 */
	  	reloadData:function(chartid,url,parameters,valueMapper){
	  		var obj = common.report.highcharts.object.get(chartid);
		    if(obj != null){
		    	if(common.utils.isEmpty(url)){
		    		  /*使用缓存数据进行处理*/
		    		 var data = obj.data;
		    		 return Highmap.reloadData(chartid,data,valueMapper);		    		
		    	}else{
		    		 /*进行后台数据更新操作*/
		    		 var chart = obj.chart;
		    		 Highmap.utils.mapShowLoading();
					 ...
					 if(common.utils.isEmpty(parameters)) parameters = obj.parameters;					  
			  		 ...
			  		 $.ajax({
			             ...
			             dataType: "json",
			             success: function(data){
			             	Highmap.reloadData(chartid,data,valueMapper);
			                Highmap.utils.mapHideLoading();
			                /*更新管理报表对象信息*/
			                obj = common.report.highcharts.object.get(chartid);
			                obj["url"] = url;
	  ...
	  mybatis:{
		 ...
		 * @param {} mapper 数据字段对应信息。格式为{key:'key?',value:'value?'}
	  	 ...
	  	 * @return {}
	  	 */
	  	reloadData:function(chartid,queryName,parameters,mapper,valueMapper){
	  		var obj = common.report.highcharts.object.get(chartid);
		    if(obj != null){
				var url = obj.url;				
		    	/*进行后台数据更新操作*/
		    	...
		    	if(!common.utils.isEmpty(parameters)) params["param"] = common.utils.strToJson(parameters);
		    	if(!common.utils.isEmpty(mapper)) params["mapper"] = common.utils.strToJson(mapper);
		    	return Highmap.ajax.reloadData(chartid,url,params,valueMapper);				
		    }else {
		      	  return false;
		    }
	  	}
	  ...
	  utils:{
		  ...
		  defaultOption:function(option){
			...
			var pointLoader_ = Highmap.utils.loadPointer(option.pointLoader);
		   ...
		   * 加载地图标识点属性值
		   */
		  loadPointer:function(pointOpt){
		  	var defaultPointLoader = {
				isLoadPoint:false, /*是否自动加载地图点属性*/
				isRemote:false, /*是否直接从后台加载数据*/
				localUrl:'/static/js/maps/json_mappoint/',  /*本地地图数据加载文件夹，地图文件请按 #pac#.geo.json */
				remoteUrl:'', /*远程地图URL参数，请求参数为{pac:#pac}*/
				pointName:'标识点', /*配置地图点默认属性名称*/
				pointVisible:false,  /*配置地图标识点是否默认显示(默认不显示)*/
				pointColor:'#fffc1b', /*标识点颜色*/
				tooltipTitleColor:'', /*标识点浮层标题颜色*/
				tooltipTextColor:'', /*标识点浮层名字颜色*/
				lineColor: 'black',
				lineWidth: 0.1,
				radius: 2,
		  ...
		  mapDrilldown:function(e,data,opt){
			...
			$.ajax({
				...
				success : function(json) {
					...
					if (common.utils.isFunction(funcName)) {
						pointinfo = eval(funcName + "(e,Highmap.levelid,Highmap.levarray.length)");
					}
					/*加载各层标识点属性*/
					var pl = Highmap.utils.loadPointer(pointinfo);
					...
					if (pl.isLoadPoint) {
						...
						Highmap.mapObject.addSeries({
							...
							dataLabels : {
								...
								format : '<span style="font-family:'
										+ Highmap.fontFamily
										+ ';font-size: 10px;">{point.properties.name}</span>'
							},
							animation : true,
							visible : pl.pointVisible,
							tooltip : {
								headerFormat : '<p><span style="font-family:'
										+ Highmap.fontFamily
										+ ';font-size: 12px;color:'
										+ pl.tooltipTitleColor
										+ '">{series.name} : </span><br/>',
								pointFormat : '<span style="font-family:'
										+ Highmap.fontFamily
		  ...
		  /**
			 ...
			 *            标识点的ID值
			 */
		  loadTopMappoint:function(opt,pointid){ 
		  	 Highmap.utils.mapShowLoading();
		  	 var pointName = opt.pointLoader.pointName;
		  	 var key = Highmap.levelid;
		  	 var url = '';
	         var parameters = {};
	         /* 读取地图数据读取器信息 */
	         if(opt.pointLoader.isRemote){
	             url = ctx + opt.pointLoader.remoteUrl;
	             parameters = {pac:key};
	         }else{
	             url = ctx + opt.pointLoader.localUrl + key + ".geo.json";
	         }
		  	 $.ajax({
				type : "GET",
				url : url,
				...
				success : function(json) {
							...
							Highmap.mapObject.addSeries({
						                ...
						                data: Highcharts.geojson(json, 'mappoint'),
						                showInLegend:true,
						                color:opt.pointLoader.pointColor,
            							marker: {
										   lineColor: opt.pointLoader.lineColor,
										   lineWidth: opt.pointLoader.lineWidth,
										   radius: opt.pointLoader.radius
										},
						                dataLabels: {
						                    align: 'left',
						                    verticalAlign: 'middle',
						                ...
						                visible:false,
						                tooltip: {
						                	headerFormat: '<p><span style="font-family:'+Highmap.fontFamily+';font-size: 12px;color:'+opt.pointLoader.tooltipTitleColor+'">{series.name} : </span><br/>',
							                pointFormat: '<span style="font-family:'+Highmap.fontFamily+';font-weight:bold;font-size: 13px;color:'+opt.pointLoader.tooltipTextColor+'">{point.name}</span></p>'
						                }
									});
						}
					});
