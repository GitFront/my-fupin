...
var Highchart = {	
	  ...
	  ChartDataFormate: {
	      ...
	      FormatTimeSeriesData: function (data,opt) {
	          ...
	          var groups = new Array();
	          var series = new Array();
	          var irregular = opt.time_irregular;
	          var type = 'area';
	          for (var i = 0; i < data.length; i++) {
	              if (!groups.contains(data[i].group))
	                  groups.push(data[i].group);
	              ...
	              if(irregular){
		             ...
		             temp_series = { 
		              ...
		              temp_series = { 
		              	...
		              	pointInterval:opt.time_pointInterval,
		              	data: temp_data,
		              	visible:visible
		              };
	              }
	  ...
	  ChartOptionTemplates: {
	      ...
	      Pie: function (container,data, opt) {
	          ...
	          var option = {
	              ...
	              colors: opt.color,
	              exporting: { enabled:opt.exporting },
	              plotOptions: {
	                  pie: {
	                      allowPointSelect: true,
	                      cursor: opt.cursor,
	                      dataLabels: {
	                          enabled: opt.dataLabels,
	                          distance: opt.pie_distance
	                      },
		                  borderWidth:0,
	                      showInLegend: true,
	                      size:opt.pie_size,
	                      startAngle: opt.pie_startAngle,
				          endAngle: opt.pie_endAngle,
	      ...
	      TimeSeriesLine:function(container,data,opt){
	         ...
	         var option = {
		        ...
		        xAxis: {
			        ...
			        visible:opt.xVisible,
		            allowDecimals:opt.xAllowDecimal,
		            tickInterval: opt.time_minRange
		        },
		        yAxis: {
		            title: {
	                   text: opt.yName || ''
	      ...
	      Gauge:function(container,data,opt){
	      	  ...
	      	  var option = {
			        ...
			        series: [{
			            ...
			            data: data,
			            dataLabels: {
			                format: '<div style="text-align:center"><span style="font-size:'+opt.gauge_valueSize+'px;color:' +
			                    ((Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black') + '">{y}</span> ' +
			                       '<span style="font-size:'+(opt.gauge_valueSize/5)*3+'px;color:silver">'+opt.yUnit+'</span></div>'
			            },
			            tooltip: {
	  ...
	  create:{
	  	...
	  	init:function(container,data, option,type){
			  ...
			  /*动态设置数据的步长信息*/
			  option_["time_minRange"] = Highchart.utils.getTimeSeriesMinRange(data);
			}
	  ...
	  reload: function (chartid,data,newOption,type) {
	      ...
	      if(obj != null){
			  ...
			  /*判断是否需要从前端统计数据*/
	  		  ...
	  		  if(type === 'TimeSeriesLine') {
			  	 /*动态设置数据的步长信息*/
			  	 opt["time_minRange"] = Highchart.utils.getTimeSeriesMinRange(data);
			  } 
	  ...
	  utils:{
		  ...
		  defaultOption:function(option){
			...
			var defaultOpt = {
				...
				title_x:0, /* 报表标题x坐标位移 */
				title_y:0, /* 报表标题y坐标位移 */
				color:Highchart.colors, /*设置图形颜色分布*/
				bars_horiz : false, /* 柱形图是否需要横向显示 */
				...
				xVisible:true, /*x轴是否显示刻度*/
				yName : '', /* y轴名称(默认为空) */
				yMin : 0, /* y轴最少值(默认为0) */
				yUnit : '', /* y轴单位(默认为空) */
				yDecimal : 0, /* y轴数据精度(默认为不保留小数位){最高只支持4位小数} */
				yAllowDecimal:true, /*y轴是否允许显示小数(默认为true)*/
				yVisible:true, /*y轴是否显示刻度*/
				stats : false, /* 是否需要进行前端数据汇总(默认为false) */
				pie_size:'85%', /*饼状图持有：调整饼图的大小*/
				pie_distance: 30, /*饼状图持有：调整数据标签的距离*/
				pie_startAngle: 0, /*饼状图持有：饼图的开始角度*/
            	pie_endAngle: 360, /*饼状图持有：饼图的结束角度*/
            	pie_center: ['50%', '50%'],  /*图形的位移比例*/
            	pie_innerSize:'0%', /*饼图特有：图形内圆所占比例*/
				stack_isPercent:false, /* 是否显示百分比数据(默认为false)-堆积图特有 */
				percentDecimal : 2,/* 显示百分比数据精度(默认为保留2位小数){最高只支持4位小数} */
				dataLabels:false, /* 是否显示数据标签(默认为false) */
				legend : true, /* 是否显示图例(默认为true) */
				zoomType : '', /* 开启哪个轴的放大缩小功能'x'/'y'(默认不开启) */
				time_minRange : 14 * 24 * 3600000, /* 时序图的x轴间隙时间为多少(默认为14天) */
				time_pointInterval : 24 * 3600 * 1000, /* 时序图每个数据点的间隙时间为多少(默认为1天) */
				time_irregular : true, /* 时序图数据点是否不连续(默认为不连续数据)，为true时，数据可以为断续数据；为false时，数据只能为连续数据 */
				gauge_max : 100, /* 仪表盘最大值(默认为100) */
				gauge_min : 0, /* 仪表盘最小值(默认为0) */
				gauge_stops : [0.4, 0.6, 0.8], /* 仪表盘颜色步长设置，输入数组，百分比数据(默认为[0.4,0.6,0.8]) */
				gauge_greatValue : 'up', /*仪表盘颜色分布设置(默认为'up')；up:大值优良，分布从左至右为红/黄/绿；down:小值优良，分布从左至右为绿/黄/红*/
				gauge_valueSize:12, /*仪表盘显示数值的大小*/
				exporting : true, /*是否启用图形的导出功能*/
			...
			/* 转换柱形图显示方式 */
			if (option_.bars_horiz) {
				option_["bars_horiz"] = 'bar';
			} else {
				option_["bars_horiz"] = 'column';
			}
			/*转换图表步长信息*/
			option_["gauge_stops"] = this.convertStops(option_.gauge_stops,
					option_.gauge_greatValue);
		  ...
		  initSeries:function(chart,data,opt,type){
			...
			switch (type)
				...
				case 'StackBars':
				  ...
				  break;
				case 'TimeSeriesLine':
				  var line_datas = Highchart.ChartDataFormate.FormatTimeSeriesData(data,opt.time_pointInterval);
				  $.each(line_datas.series, function(itemNo, series_) {
				  	chart.addSeries(series_,false);
				  });
				  break;
