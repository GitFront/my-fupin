  ...
  function getDate(){
   ...
   var queryName="com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryEcMapData";
    $(function(){
    	...
    	if(ret){ret.then(function(res){
    		  ...
    		  chartid = Highmap.mybatis.Map("map",queryName,{"type":type,"scope":scope,"month":statTime},
    		  	{key:'PAC',value:'COMPLETE_MODEL_NUM',value2:'NATURAL_NUM',value3:'COMPLETE_MODEL_RATE'},"countries/cn/custom/cn-gongdong","PAC",option_);
    ...
    function loadMap(type){
		...
		Highmap.mybatis.reloadData(chartid,queryName,{"type":type,"scope":scope,"month":statTime},mapper,valueMapper,colorStops);
    }
    	...
    	var loader = {};
		 if(level < 5) {
			 ...
			 var point1 = {
					 ...
					 parameters:{"month":statTime,"buildStatus":"达标"},
			 ...
			 var point2 = {
					 queryName:'com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryMappiontGeoForCountry', /*通过远程方式读取，读取的查询名称（mybatis）读取的查询参数为pac:区域ID，level:区域等级 */
					 parameters:{"month":statTime,"buildStatus":"建设中"},
					 mapper:{name:'NAME',PAC:'PAC',PPAC:'PPAC',longitude:'LONGITUDE',latitude:'LATITUDE',type:'POINT_TYPE'}, /*通过远程方式读取，读取的查询转义*/
			 ...
			 var point3 = {
					 queryName:'com.aspire.birp.modules.antiPoverty.dao.ExampleCountryMapDao.queryMappiontGeoForCountry', /*通过远程方式读取，读取的查询名称（mybatis）读取的查询参数为pac:区域ID，level:区域等级 */
					 parameters:{"month":statTime,"buildStatus":"未启动"},
					 mapper:{name:'NAME',PAC:'PAC',PPAC:'PPAC',longitude:'LONGITUDE',latitude:'LATITUDE',type:'POINT_TYPE'}, /*通过远程方式读取，读取的查询转义*/
