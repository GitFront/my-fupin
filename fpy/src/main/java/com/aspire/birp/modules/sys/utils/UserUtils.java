...
public class UserUtils  {
	...
	public static Set<String> getAreaList() throws RemoteException{
		Set<String>areaSet=Collections.synchronizedSet(new HashSet<String>());
		if(UserUtils.getUser().isAdmin()){
			areaSet.add("440000000000");
			return areaSet;
		}
		
		
		List<Area> areaList = getSecurityService().findAreaList(getUserLoginName());
		for (Area area : areaList) {
			areaSet.add(area.getId());
		}
		return areaSet;
	}
	 ...
	 * @param params
	 */
	public static void putHelpPacToParams(Map<String, Object> params){
		User user = getUser(); //获取用户
		List<Role> userRole = user.getRoleList();
		String helpPac = userRole.size() > 0 ? userRole.get(0).getRemarks() : null;
		if(null == helpPac || "".equals(helpPac)){
			params.put("helppac", "999");
		}else{
			params.put("helppac", helpPac);
		}
	}
